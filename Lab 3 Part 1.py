...
def create_segments(dataframe):
    ...
    df_persona["SOURCE"] = pd.qcut(df_persona["PRICE"], 4, labels=segment_labels)
    df_segment = df_persona.groupby("SOURCE").agg({"PRICE": "mean"})

    # Demonstrating segments as bars on a chart, where the length of each bar varies based on the value of the customer profile
    plot = sns.scatterplot(x="SOURCE", y="PRICE", data=df_segment)
    for scatter in plot.patches:
        plot.annotate(format(scatter.get_height(), '.2f'),
                      (scatter.get_x() + scatter.get_width() / 2,
                       scatter.get_height()), ha='center', va='center',
                      size=8, xytext=(0, 8),
...
def ruled_based_classification(dataframe):
    ...
    print(new_user)
    print("Segment:" + df_segment[df_segment["CUSTOMERS_LEVEL_BASED"] == new_user].loc[:, "SOURCE"].values[0])
